"""Full mHC-Path model: DINOv3 backbone -> FPN -> RF-DETR decoder.

Assembles the complete detection/segmentation pipeline with a frozen
DINOv3 backbone, multi-scale FPN, and RF-DETR decoder with deformable
attention and iterative box refinement.
"""
...
class MHCPathConfig:
    ...
    Attributes:
        backbone: Model variant key (e.g. ``"dinov3_vitl16"``).
        ...
        mask_upsample_factor: Spatial upsample factor for mask head.
...
class MHCPath(nn.Module):
    ...
    Composes a frozen DINOv3 backbone, a pathology FPN, and an RF-DETR
    decoder with deformable attention and iterative box refinement.
    """
    ...
    def forward(self, images: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return self.decoder(fpn_features)
